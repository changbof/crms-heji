...
$current_user_name=UserSession::getUserName();

if (Common::isPost()) {
	if($change_password){
		$ret=User::checkPassword($current_user_name,$old);
		if($ret){
			if(strlen($new)<6){
				...
				$usrPwd = md5($new);
				$user_data['password'] = $usrPwd;
				$imuser_data['userpass'] = $usrPwd;
				User::updateUser ( $current_user_id, $user_data );
				//更新webin中的密码
				ImUser::updateUserByUserName($current_user_name,$imuser_data);
