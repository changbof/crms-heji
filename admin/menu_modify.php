...
$menu_id = $menu_name = $menu_url = $module_id = $is_show =$online = $shortcut_allowed = $menu_desc = $father_menu = $menu_sort = '';
	...
	if($menu_name == "" || $menu_url =="" || ($menu_id>100 && empty($module_id)) ){
		...
		if(!$exist){
			$update_data = array ('menu_name' => $menu_name, 'menu_url' => $menu_url,  
								'is_show' => $is_show, "online" => $online,'menu_desc' => $menu_desc, 'shortcut_allowed' => $shortcut_allowed,'father_menu' => $father_menu,'menu_sort' => $menu_sort);
			if($menu_id > 100){
