...
$menu_name = $menu_url = $module_id = $is_show = $online = $shortcut_allowed = $menu_desc = $father_menu = $menu_sort = '';
	...
	if($menu_name=="" || $menu_url == ""|| $module_id == ""){
		...
		if(!empty($menu)){
			...
			$input_data = array ('menu_name' => $menu_name, 'menu_url' => $menu_url, 'module_id' => $module_id, 
								'is_show' => $is_show, 'online' =>1 , 'menu_desc' => $menu_desc ,'shortcut_allowed'=>$shortcut_allowed,'father_menu'=>$father_menu,'menu_sort' => $menu_sort );
