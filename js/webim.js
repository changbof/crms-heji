...
function TempChat()
{
	new Ajax().get("data/gettempemail.php",function(o){
...
function WebIM()
{
		...
		showContextMenu:function(o,evt)
		{
			...
			if(parseInt(e.button)==2)//右键菜单
			{
				...
				if(uid)
				{
					...
					arr.add("发送电子邮件|_webIM.Common.showLink(250,200,(Elem.Height()-200)/2,(Elem.Width()-250)/2,11,'发送电子邮件','page/sendemail.php?e="+u.UserEmail+"')");
					arr.add("");
	...
	this.Data = {
		...
		{
			new Ajax().get("data/getmymsg.php?rnd="+Math.random()+"&code="+_me.Sys.Code,cb);
		},
		//修改用户Profile
	...
	this.CMD = {
		...
		showRegWindow:function()
		{
			...
			_me.Common.showLink(350,320,50,(Elem.Width()-350)/2,7,"注册新帐号","page/reg.php?v="+Math.random(),null,
				function()
				{
		...
		intLoginWindow:function()
		{
			...
			w.LoadedCallback = function(id,t)
			{
				...
				if(Other.GetCookie("stremail").trim()!=""){
					Elem.Show($("linkDelCookieId6Type3").parentNode);
				}
		...
		{
			var w = new WebForm();
			w.Title = "<span style='color:#fff'>"+ _wTitle + _me.Version+"</span>";
			w.Icon = "mainicon.gif";
			w.Type = 2;
			w.Height = 450;
			w.Width = 210;
					...
					var intInteralID = setInterval(
						function()
						{
							if(_me.Config&&_me.Profile&&_me.Group&&_me.Friend)
							{
								...
								{
									_me.Common.showLink(350,285,w.Top,w.Left<355?w.Left+w.Width+2:w.Left-352,1,"添加联系人","page/addfriend.php?v="+Math.random(),"tooladdfriend.gif");
								};
								$("wMainOptionId"+id+"Type"+t).onclick = function()//选项窗口
								{
									_me.Common.showLink(420,400,w.Top,w.Left<425?w.Left+w.Width+2:w.Left-422,8,"选项","page/option.php?v="+Math.random(),"tooloption.gif");
								};
								$("wMainProfileId"+id+"Type"+t).onclick = function()//个人信息窗口
								{
									_me.Common.showLink(350,300,w.Top,w.Left<355?w.Left+w.Width+2:w.Left-352,10,"编辑个人资料","page/profile.php?v="+Math.random(),"toolprofile.gif");
								};
								...
								{
									_me.Common.showLink(400,400,w.Top,w.Left<405?w.Left+w.Width+2:w.Left-402,2,"今日焦点","page/todayfocus.php?v="+Math.random(),"toolshowfocus.gif");
								};
