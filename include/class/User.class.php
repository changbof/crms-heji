...
class User extends Base{
	...
	// 查询字段
	private static $columns = 'user_id, user_name, password, real_name, mobile, email, user_desc, login_time, status, login_ip, user_group, template,shortcuts,show_quicknote, last_online_time, online, ext';
	//状态定义
	const ACTIVE = 1;
	const DEACTIVE = 0;
	...
	public static function logout(){
		...
		self::updateUser(UserSession::getUserId(),$user_data);
	...
	public static function getUsersByGroup( $group_ids ) {
		$db=self::__instance();
		$condition = array();
		if(!is_array($group_ids)){
			$group_ids=explode(',',$group_ids);
			$condition = array( "user_group" => $group_ids );
		}
	...
	public static function getUserForOptions($group_id='') {
		if($group_id==''){
			$user_list = self::getAllUsers();
		}else{
			$user_list = self::getUsersByGroup($group_id);
		}
		foreach ( $user_list as $user ) {
			$user_options_array [$user ['user_id']] = $user ['real_name'];
		}
		
		return $user_options_array;
	}
	...
	*/
	...
	public static function setExt($user_id,$ext){
		$user_data=array("ext"=>$ext);
		$ret=self::updateUser($user_id,$user_data);
		return $ret;
	}
	public static function setOnline($user_id,$online){
		$user_data=array("ext"=>$online);
		$ret=self::updateUser($user_id,$user_data);
		return $ret;
	}
	public static function loginDoSomething($user_id){
		...
		$update_data = array ('login_ip' => $login_ip, 'login_time' => $login_time, 'online' => 'online' );
		...
		//获取上一次养护期客户检查时间
		$checkTime = SystemParam::getValue("checkvalid_date");
		if($checkTime){
				$user_info['checkvalid_date'] = $checkTime['param_value'];
		}else{
				$user_info['checkvalid_date'] = 0;
		}
